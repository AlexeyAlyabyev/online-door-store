...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'color_image'      => $query->row['color_image'],
				'compressed_image' => $query->row['compressed_image'],
	...
	public function getProducts($data = array()) {
		...
		if (isset($data['start']) || isset($data['limit'])) {
			...
			if (!isset($data['start']) || $data['limit'] < 1) {
				$data['limit'] = 20;
			}
	...
	public function getProductMainCategory($product_id) {
		$query = $this->db->query("SELECT category_id FROM `oc_product_to_category` WHERE `product_id` = ". $product_id ." AND main_category = 1");
	...
	public function getProductRelatedByColor($product_id) {
		...
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related_by_color pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_product_by_color_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");

		foreach ($query->rows as $result) {
			$product_data[$result['related_product_by_color_id']] = $this->getProduct($result['related_product_by_color_id']);
		}

		return $product_data;
	}

	public function getProductRelatedByGlazing($product_id, $glazing_attribute_id) {
		$product_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_related_by_glazing pr LEFT JOIN " . DB_PREFIX . "product p ON (pr.related_product_by_glazing_id = p.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pr.product_id = '" . (int)$product_id . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");

		foreach ($query->rows as $result) {
			$product_data[$result['related_product_by_glazing_id']] = $this->getProductAttributeValue($result['related_product_by_glazing_id'], $glazing_attribute_id);
		}
	...
	public function getProductAttributeValue($product_id, $attribute_id){
		...
		isset($query->row['text']) ? $data = $query->row['text'] : $data = "";
