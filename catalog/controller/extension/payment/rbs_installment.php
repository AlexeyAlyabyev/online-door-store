...
class ControllerExtensionPaymentRBSInstallment extends Controller
{
    ...
    public function __construct($registry)
    {
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/rbs_installment.twig')) {
    ...
    public function index()
    {
        ...
        $rbs_installment_max_month = $this->config->get('payment_rbs_installment_max_month');

        if ($rbs_installment_max_month > 0){
            $min_payment = number_format(round($amount / 100 / $rbs_installment_max_month, 2), 2, '.', ' ');
        }

        $data['amount_string'] = number_format($amount / 100, 2, '.', ' ');
        $data['min_payment_string'] = $min_payment;
        $data['payment_link'] = $result['formUrl'];

        $data['action'] = $this->url->link('extension/payment/rbs_installment/payment');
        $data['button_confirm'] = $this->language->get('button_confirm');
        return $this->get_template('extension/payment/rbs_installment', $data);
    }
    ...
    private function initializeRbs()
    {
        ...
        $this->rbsCreditLibrary->login = $this->config->get('payment_rbs_installment_merchant_login');
        $this->rbsCreditLibrary->password = $this->config->get('payment_rbs_installment_merchant_password');
        $this->rbsCreditLibrary->mode = $this->config->get('payment_rbs_installment_mode');
        ...
        $this->rbsCreditLibrary->logging = $this->config->get('payment_rbs_installment_logging');

        $this->rbsCreditLibrary->returnUrl = $this->config->get('payment_rbs_installment_returnUrl');
        $this->rbsCreditLibrary->failUrl = $this->config->get('payment_rbs_installment_failUrl');
    }
