...
class ControllerExtensionQuickCheckoutConfirm extends Controller { 
	public function index() {
		...
		if (!$redirect) {
			...
			// Создание лидов в битрикс при заказе товара через корзину

			$this->load->model('catalog/product');

			$order_text = "Способ доставки: ".$order_data['shipping_method'].";<br>";
			$total_price = 0;
			...
			foreach ($order_data['products'] as $product) {
				...
				$order_text .= $product['name'].$color." - ".$product['quantity'].";<br>";
			...
			$queryData = http_build_query(array(
				'fields' => array(
					'NAME' => $order_data['firstname'],   // сохраняем имя
					"PHONE" => array(array('VALUE' => $order_data['telephone'], 'VALUE_TYPE' => 'WORK')),
					"EMAIL" => array(array('VALUE' => $order_data['email'], 'VALUE_TYPE' => 'WORK')),
