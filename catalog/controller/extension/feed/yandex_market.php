...
class ControllerExtensionFeedYandexMarket extends Controller {
	...
	public function index() {
		if ($this->config->get('feed_yandex_market_status')) {
			...
			// ----------------------------------------CUSTOM CODE START-------------------------------
			// Подключаем работу с элементами каталога 
...
//				$data['cbid'] = 15;
				...
				// ----------------------------------------CUSTOM CODE START-------------------------------
				// При наличии у двери только числового названия, добавляем к имени цвет, чтобы ЯМ не ругался
				// В первой строке отсеиваем пустые товары для МСК (товары есть в СПБ, но их нет в МСК) 

				if (!(int)$product['price']) continue;
				if (is_numeric($product['name'])) {
					$attribute_groups = $this->model_catalog_product->getProductAttributes($product['product_id']);
					foreach ($attribute_groups[0]['attribute'] as $attribute_group){
						if ($attribute_group['attribute_id'] == 12) $product['name'] = $product['name']." ".$attribute_group['text'];
						if ($attribute_group['attribute_id'] == 51) $product['name'] = $product['name']." ".$attribute_group['text'];
					}
				}

				// ----------------------------------------CUSTOM CODE END-------------------------------

				// Добавляем список аттрибутов в характеристики товара
				$attributes = $this->model_catalog_product->getProductAttributes($product['product_id']);
				
				if (isset($attributes[0])) {
					$attributes = $attributes[0]['attribute'];
					foreach ($attributes as $key => $attribute){
						if ($attribute['attribute_id'] == 54) unset($attributes[$key]);
					}
...
//				$data['local_delivery_cost'] = 100;
				...
				// if ($product['name'] = 'Межкомнатная дверь Linea 1') print_r($product['description']);
...
//		}
	...
	private function array2Param($params) {
		...
		foreach ($params as $param) {
			...
			$retval .= '">' . $this->prepareField($param['text']) . '</param>' . $this->eol;
		}
