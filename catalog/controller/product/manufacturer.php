...
class ControllerProductManufacturer extends Controller {
	...
	public function info() {
		...
		if ($manufacturer_info) {
			...
			// Last-Modified
			$template_name = "product/manufacturer_info";
			$template_path = DIR_TEMPLATE . $this->config->get('theme_default_directory') . "/template/"  . $template_name . ".twig";
			$LastModified_unix = max(strtotime($manufacturer_info['date_modified']) - 10800, filectime($template_path), filectime(__FILE__));
			...
			$this->document->setTitle('Товары от производителя '.$manufacturer_info['name'].' купить недорого в Москве');
			$this->document->setDescription('Товары от производителя '.$manufacturer_info['name'].' купить в Москве и области ✔Качественная установка ✔Недорого ✔Всегда скидки и акции ☎+7 (495) 021-55-32 ✔100 Дверей');
			$url = '';
			...
			$data['heading_title'] = 'Товары от производителя '.$manufacturer_info['name'];
			...
			$data['results'] = sprintf($value_text_pagination, ($product_total));
