...
class ControllerCatalogProduct extends Controller {
	...
	public function editFast() {
		$this->load->model('catalog/product');

		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$this->model_catalog_product->editProductFast($this->request->post);
		}
	}
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			...
			foreach ($product_specials  as $product_special) {
				if (($product_special['date_start'] == '0000-00-00' || strtotime($product_special['date_start']) < time()) && ($product_special['date_end'] == '0000-00-00' || strtotime($product_special['date_end']) > time())) {
					...
					$special = ceil($product_special['price']);
		...
		$current_manufacturer = $this->model_catalog_manufacturer->getManufacturer($filter_manufacturer);
		foreach ($manufacturers as $key => $manufacturer){
			if ($manufacturer == $current_manufacturer) 
				$manufacturers[$key]['selected'] = true;
			else 
				$manufacturers[$key]['selected'] = false;
		}
	...
	protected function getForm() {
		...
		$data['href_shop'] = HTTP_CATALOG . 'index.php?route=product/product&product_id=' . $data['product_id'];

		$data['user_token'] = $this->session->data['user_token'];

		$this->load->model('localisation/language');

		$data['languages'][] = $this->model_localisation_language->getLanguages()[$this->config->get('config_language')];
		...
		} elseif (!empty($product_info) && strpos($product_info['image'], ".webp") != false) {
		...
		// Color_image
		if (isset($this->request->post['color_image'])) {
			$data['color_image'] = $this->request->post['color_image'];
		} elseif (!empty($product_info)) {
		...
		if (isset($this->request->post['color_image']) && is_file(DIR_IMAGE . $this->request->post['color_image'])) {
			$data['color_thumb'] = $this->model_tool_image->resize($this->request->post['color_image'], 100, 100);
		} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['color_image'])) {
			$data['color_thumb'] = $this->model_tool_image->resize($product_info['color_image'], 100, 100);
		} else {
			$data['color_thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}

		// Определяем домен для копирования оригинала фотки товара в буфер обмена
		$data["site_imgs_url"] = "https://" . $_SERVER['SERVER_NAME'] . "/image/";

		// Compressed_image
		if (isset($this->request->post['compressed_image'])) {
			$data['compressed_image'] = $this->request->post['compressed_image'];
		} elseif (!empty($product_info)) {
			$data['compressed_image'] = $product_info['compressed_image'];
		} else {
			$data['compressed_image'] = '';
		}

		if (isset($this->request->post['compressed_image']) && is_file(DIR_IMAGE . $this->request->post['compressed_image'])) {
			$data['compressed_thumb'] = "../image/" . $this->request->post['compressed_image'];
		} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_info['compressed_image'])) {
			$data['compressed_thumb'] = "../image/" . $product_info['compressed_image'];
		} else {
			$data['compressed_thumb'] = $this->model_tool_image->resize('no_image.png', 100, 100);
		}
		...
		foreach ($product_images as $product_image) {
			...
			if (is_file(DIR_IMAGE . $product_image['image'])) {
				if (!empty($product_image['image']) && strpos($product_image['image'], ".webp") != false) {
					$thumb = "../image/" . $product_image['image'];
				} elseif (!empty($product_info) && is_file(DIR_IMAGE . $product_image['image'])) {
		...
		if (isset($this->request->get['product_id'])) {
			$products = $this->model_catalog_product->getProductRelatedByGlazing($this->request->get['product_id']);
		} else {
			$products = array();
		}
		...
		if (isset($this->request->get['product_id'])) {
			$products = $this->model_catalog_product->getProductAccessories($this->request->get['product_id']);
		} else {
			$products = array();
		}
