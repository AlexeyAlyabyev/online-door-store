...
class ControllerCatalogCategory extends Controller {
	...
	public function editFast(){
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			$this->model_catalog_category->editProductFast($this->request->post);
		}
	}
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_name'])) {
			$data['filter_name'] = $filter_data['filter_name'] = $this->request->get['filter_name'];
		}

		if (isset($this->request->get['nesting_level'])) {
			$data['nesting_level'] = $filter_data['nesting_level'] = $this->request->get['nesting_level'];
		}
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				...
				'href_shop'  	=> HTTP_CATALOG . 'index.php?route=product/category&category_id=' . $result['category_id'],
		...
		if (isset($this->request->get['nesting_level'])) {
			$url .= '&nesting_level=' . $this->request->get['nesting_level'];
		}
	...
	protected function getForm() {
		...
		isset($this->request->get['category_id']) ? $data['href_shop'] = HTTP_CATALOG . 'index.php?route=product/category&category_id=' . $this->request->get['category_id'] : $data['href_shop'] = "";
		...
		if (isset($this->request->get['category_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			...
			// Определяем какой именно из фильтров используется в данной категории
			$layout = $this->model_catalog_category->getCategoryLayouts($this->request->get['category_id']);
			$this->load->model('design/layout');
			$layout_modules = $this->model_design_layout->getLayoutModules(end($layout));
			foreach ($layout_modules as $layout_module){
				if (strpos($layout_module['code'], 'dream_filter') !== false) {
					$dream_filter_id = explode('.', $layout_module['code']);
				}
			}
			// Если получилось определить связанный модуль, получае данные фильтров по нему
			if (isset($dream_filter_id)) {
				$dream_filter_id = end($dream_filter_id);

				$this->load->model('setting/module');
				$module_info = $this->model_setting_module->getModule($dream_filter_id);

				$attributes_list = array();
				$options_list = array();

				foreach ($module_info['filters'] as $filter){
				...
				$data['manufacturers_list'] = (int)$this->model_design_seo_filter->countSeoFilterManufacturers() == true;

				// Оставляем в отборе только атрибуты по которым присутствуют сео-ключи
				$active_attributes = $this->model_design_seo_filter->getActiveSeoFilterAttributes();
				foreach ($active_attributes as $key => $item){
					$active_attributes[$key] = $item['attribute_id'];
				}
				$attributes_list = array_intersect($attributes_list, $active_attributes);
				...
				// Оставляем в отборе только атрибуты по которым присутствуют сео-ключи
				$active_options = $this->model_design_seo_filter->getActiveSeoFilterOptions();
				foreach ($active_options as $key => $item){
					$active_options[$key] = $item['option_id'];
				}
				$options_list = array_intersect($options_list, $active_options);
		...
		// -------------------------------------		CUSTOM CODE START 		-------------------------------------
		// Записываем значения заголовка и текста баннера для передачи в шаблон
